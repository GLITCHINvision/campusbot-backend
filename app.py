...
def chat():
    ...
    try:
        ...
        if not user_query:
            return jsonify({"reply": "⚠️ No query received."}), 400
        ...
        return jsonify({"reply": "⚠️ Invalid input received."}), 400

    except Exception as unexpected:  # pylint: disable=broad-exception-caught
        print("❌ Unknown error:", str(unexpected))
        return jsonify({"reply": "⚠️ Internal server error occurred."}), 500
